...
struct reads_config
{
    ...
    uint8_t errors{2u};
    uint32_t read_length{250u};
    uint32_t number_of_reads{1ULL << 20};
    uint16_t number_of_haplotypes{1u};
};

void reads_generator(reads_config const & cfg)
{
    ...
    seqan3::sequence_file_output fout{(cfg.out_file)};

    for (std::filesystem::path const & bin_file : cfg.bin_paths)
    {

        seqan3::sequence_file_input<my_traits, seqan3::fields<seqan3::field::seq>> fin{bin_file}; //, seqan3::type_list<seqan3::format_fasta>
        ...
        for (auto && record : fin)
        {
                 ...
                 ++current_read_number, ++read_counter, ++reads_per_bin_counter)
            {
                ...
                auto read_span = std::span{seq.data() + read_start_pos, cfg.read_length};
...
void initialise_argument_parser(sharg::parser & parser, reads_config & cfg)
{
    ...
    parser.add_option(cfg.out_file,
                      sharg::config{.short_id = '\0',
                                    .long_id = "output",
                                    .description = "Provide the file where the reads should be written to.",
                                    .required = true,
